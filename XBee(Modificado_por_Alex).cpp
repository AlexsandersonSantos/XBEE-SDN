...
ZBExpRxResponse::ZBExpRxResponse() : RxDataResponse() {
	
}

uint16_t ZBExpRxResponse::getRemoteAddress16() {
	return 	(getFrameData()[8] << 8) + getFrameData()[9];
}

uint8_t ZBExpRxResponse::getOption() {
	return getFrameData()[10];
}

// Marca onde deve-se ler o frame para pegar os dados, neste caso esta pegando na 12 posição do array
uint8_t ZBExpRxResponse::getDataOffset() {
	return 17;
}

uint8_t ZBExpRxResponse::getDataLength() {
	return getPacketLength() - getDataOffset() - 1;
}


XBeeAddress64& ZBExpRxResponse::getRemoteAddress64() {
	return _remoteAddress64;
}

void XBeeResponse::getZBExpRxResponse(XBeeResponse &rxResponse) {

	ZBExpRxResponse* zb = static_cast<ZBExpRxResponse*>(&rxResponse);
